...
for data in init_data:
    ...
    if not os.path.exists(feature_file) or not os.path.exists(prob_file):
        ...
        # init_net.load_data(sub_dir['mxnet_recs'], 'noisy_train_disturb.rec')
        # print init_net.test_acc('noisy_train_disturb.rec')
...
if test:
    ...
    prefix = quality_quantity.script(sub_dir, sub_dir['noisy_train_disturb'], data, ratio=1)

    handler = logging.FileHandler(os.path.join(sub_dir['mxnet_logs'], '%s.log' % prefix))
    logger.addHandler(handler)

    init_net = simpleNet(init_prefix, 10, n_epoch=20, mean_image=mean_img,
                         ctx=ctx)
